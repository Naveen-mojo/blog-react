    ...
    return (
        <>
            ...
            <div id="site-content" className="site-content">
                <div className="container atbs-block-custom-margin"><div className="block-heading page-heading block-heading--style-1"><h2 className="page-heading__title block-heading__title">Videos</h2></div>{/* block-heading */}</div>{/* container */}    {/* listing has sidebar */}
                                                    ...
                                                    <div className="list-item" key={index}>
                                                        <article className="post post--vertical list-large-post--vertical-large list-large-post--vertical-large-has-sidebar disable-thumb-placeholder" data-dark-mode="true">
                                                            <div className="post__thumb atbs-thumb-object-fit post-thumb-radius">
                                                                <Link to={`/${curValue.PostSlug}`}><img width={1200} height={600} src={curValue.PostThumbUrl} className="attachment-atbs-l-2_1 size-atbs-l-2_1 wp-post-image" alt="" sizes="(max-width: 1200px) 100vw, 1200px" /></Link> {/* close a tag */}                                          </div>
                                                            <div className="post__text">
                                                                <div className="post__meta post-time-cat-wrap">
                                                                    <Link className="cat-6 post__cat post__cat--bg cat-theme-bg" to={``}>Videos</Link><span className="time-wrap"><time className="time published" dateTime={moment(curValue.CreationDate).format('MMMM Do YYYY')} title={moment(curValue.CreationDate).format('MMMM Do YYYY')}>{moment(curValue.CreationDate).format('MMMM Do YYYY')}</time></span></div>
                                ...
                                <div className="atbs-sub-col atbs-sub-col--right sidebar js-sticky-sidebar" role="complementary">
                                    ...
                                    <div id="bk_widget_posts_list-2" className="widget atbs-widget">
                                        <div className="atbs-widget atbs-widget-posts-4">
                                            <div className="widget__title widget__title--style-1"><h4 className="widget__title-text">Posts List</h4></div>
                                                                ...
                                                                return (
                                                                    <li className="list-item" key={CurValue.ID}>
                                                                        <article className="post post--no-thumb post-no-thumb" data-dark-mode="true">
                                                                            <div className="post__text">
                                                                                <div className="post__meta post-time-cat-wrap">
                                                                                    <Link className="cat-6 post__cat cat-theme" to={``}>Videos</Link><span className="time-wrap"><time className="time published" dateTime={moment(CurValue.CreationDate).format('MMMM Do YYYY')} title={moment(CurValue.CreationDate).format('MMMM Do YYYY')}>{moment(CurValue.CreationDate).format('MMMM Do YYYY')}</time></span></div>
                                                                                <div className="post__text--wrap">
